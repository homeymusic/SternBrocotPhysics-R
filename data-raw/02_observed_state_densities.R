...
agg_dir <- file.path(base_data_dir_4TB, "02_observed_state_densities")
...
existing_densities <- list.files(agg_dir, pattern = "^observed_state_density_P_.*\\.csv\\.gz$")

all_p_names  <- gsub("erasures_P_([0-9.]+)\\.csv\\.gz", "\\1", basename(all_files))
done_p_names <- gsub("observed_state_density_P_([0-9.]+)\\.csv\\.gz", "\\1", existing_densities)
