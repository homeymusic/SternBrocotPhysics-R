...
agg_dir <- file.path(base_data_dir_4TB, "02_harmonic_oscillator_erasure_distance_densities")

if (!dir.exists(agg_dir)) dir.create(agg_dir, recursive = TRUE)

# Notice the added harmonic_oscillator_ prefix in the patterns
all_files <- list.files(raw_dir, pattern = "^harmonic_oscillator_erasures_P_.*\\.csv\\.gz$", full.names = TRUE)
existing_densities <- list.files(agg_dir, pattern = "^harmonic_oscillator_erasure_distance_density_P_.*\\.csv\\.gz$")

all_p_names  <- gsub("harmonic_oscillator_erasures_P_([0-9.]+)\\.csv\\.gz", "\\1", basename(all_files))
done_p_names <- gsub("harmonic_oscillator_erasure_distance_density_P_([0-9.]+)\\.csv\\.gz", "\\1", existing_densities)
