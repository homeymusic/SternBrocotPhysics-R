...
# --- 1. Configuration ---
# Split the metrics based on their physical and mathematical properties
...
for (col in cols_topological) {
  ...
  # CHANGED: Route to the new summary directory
  file_summary_out <- file.path(dir_04_summary, sprintf("harmonic_oscillator_%s_summary.csv.gz", col))

  # Find all Step 03 Node files for this specific column
  ...
  # High-Speed Aggregation: We only need the first row to grab the metadata
  ...
  if (nrow(dt_summary) > 0) {
    # Clean and Sort
    dt_summary <- dt_summary[!is.na(complexity_ntv)]
...
for (col in cols_algorithmic) {
  ...
  # Find all Step 02 Density files for this specific column
