...
//' @export
 ...
 void micro_macro_bell_erasure_sweep(
   ...
   pool.parallelFor(0, n, [&](int i) {
     ...
     // --- STRICT API COMPLIANCE HEADER ---
     // 1. Context: angle, spin
     // 2. Data API: erasure_distance -> microstate -> macrostate -> ... -> found
     ...
     for (int j = 0; j < count; j++) {
       ...
       // 1. Linearly sweep the "Vacuum Potential" (mu_particle)
       double mu_particle = mu_start + (mu_end - mu_start) * ((double)j / (double)(count - 1));

       // 2. Apply Source Constraint (delta_particle)
       EraseResult erasure = erase_single_native(mu_particle, delta_particle, max_depth);

       // 3. Define Particle as Erasure Distance (Thermodynamic Anomaly)
       // If found, the particle is the cost (distance). If not found, it is the raw vacuum fluctuation.
       double particle = erasure.found ? erasure.erasure_distance : mu_particle;

       // 4. Entanglement (Conservation)
       // The singlet splits into +Cost and -Cost components relative to the vacuum.
       double particle_a = particle;
       double particle_b = -particle;

       // --- STEP 2: MEASUREMENT (Detector Interaction) ---
       // The Detector interacts with the Particle relative to its own Angle.
       double phase_a = angle_rad - particle_a;
       double phase_b = angle_rad - particle_b;

       // Detector Uncertainty (Geometric Aperture)
       ...
       // We erase the *Particle* (as seen by the detector)
