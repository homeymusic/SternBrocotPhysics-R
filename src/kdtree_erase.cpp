...
EraseResult kdtree_erase_single_native(double blob_center, double squeezed_boundary, int max_sequence_length) {
  ...
  double selected_microstate = (left_bound + right_bound) / 2.0;
  ...
  while (abs_displacement > squeezed_boundary) {
    ...
    if (bit_is_one) {
      left_bound = selected_microstate;
      numerator = (numerator * 2) + 1;
      one_count++;
    } else {
      right_bound = selected_microstate;
      numerator = (numerator * 2) - 1;
      zero_count++;
    }
    ...
    selected_microstate = (left_bound + right_bound) / 2.0;
...
DataFrame kdtree_erase_core(NumericVector blob_center, double squeezed_boundary, int max_search_depth) {
  ...
  for (int i = 0; i < n; ++i) {
    EraseResult res = kdtree_erase_single_native(blob_center[i], squeezed_boundary, max_search_depth);
...
DataFrame kdtree_erase_squeezed_boundary(NumericVector x, double squeezed_boundary) {
  ...
  return kdtree_erase_core(x, squeezed_boundary, 20000);
}
...
DataFrame kdtree_erase_squeezed_boundary_and_depth(NumericVector x, double squeezed_boundary, int depth) {
  return kdtree_erase_core(x, squeezed_boundary, depth);
}
