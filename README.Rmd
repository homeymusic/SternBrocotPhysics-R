...
# PASS 1: Get exact eigenstates (original logic)
dt_eigen_raw <- plot_nodes_p[is_eigenstate == TRUE]
dt_eigen_exact <- get_median_by_nodes(dt_eigen_raw)

# PASS 2: For missing node counts, find closest approximations
max_n_found <- max(plot_nodes_p$nodes, na.rm = TRUE)
all_nodes <- data.table::data.table(n = 0:max_n_found)

# Identify which nodes are missing exact eigenstates
...
p_momentum_hybrid <- ggplot(plot_nodes_p, aes(x = p_val, y = nodes)) +
  ...
  labs(
    ...
    subtitle = latex2exp::TeX("Red circles: Exact eigenstates ($P = n\\pi$) | Orange triangles: Median P for missing states | Blue line: Ideal $P = n\\pi$"),
    x = latex2exp::TeX("Momentum P [$p/p_0$]"),
    y = latex2exp::TeX("Node Count (Coordinate $Q$ Distributions)"),
    caption = "Red: Exact Eigenstates | Orange: Median Approximations | Grey: All Configurations"
  ) +
  theme_minimal() +
  theme(
...
# Create dataset for plotting distributions (hybrid approach)
