...
render_combined_observed_state_grid <- function(matches_dt) {
  ...
  for (i in seq_len(nrow(matches_dt))) {
    ...
    if (file.exists(f_path)) {
      ...
      if (nrow(dt_found) > 0) {
        # Strictly using 'macrostate' as requested
        p_scat <- ggplot(dt_found, aes(x = microstate, y = macrostate)) +
          geom_point(alpha = 0.25, size = 0.1, color = "black") + 
          theme_minimal() +
          theme(axis.title = element_blank(), 
...
render_macro_erasure_scatters <- function(matches_dt) {
  for (i in seq_len(nrow(matches_dt))) {
    ...
    if (file.exists(f_path)) {
      ...
      p <- ggplot(dt_found, aes(x = macrostate, y = erasure_distance)) +
        geom_point(alpha = 0.4, size = 0.5, color = "black") +
        labs(title = sprintf("Phase Space Erasure: P = %.6f", q_val),
             subtitle = sprintf("Nodes: %d | Range: [%.3f, %.3f]", 
                               row$node_count, min(dt_found$microstate), max(dt_found$microstate)),
             x = "Macrostate", y = "Erasure Distance") +
        theme_minimal() +
        theme(plot.title = element_text(family = "mono", face = "bold"))
...
render_erasure_macro_scatters <- function(matches_dt) {
  for (i in seq_len(nrow(matches_dt))) {
    ...
    if (file.exists(f_path)) {
      ...
      p <- ggplot(dt_found, aes(x = erasure_distance, y = macrostate)) +
        geom_point(alpha = 0.4, size = 0.5, color = "black") +
        labs(title = sprintf("Phase Space Erasure: P = %.6f", q_val),
             subtitle = sprintf("Nodes: %d | Range: [%.3f, %.3f]", 
                               row$node_count, min(dt_found$microstate), max(dt_found$microstate)),
             x = "Erasure Distance", y = "Macrostate") +
        theme_minimal() +
        theme(plot.title = element_text(family = "mono", face = "bold"))
