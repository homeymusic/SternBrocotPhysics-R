...
generate_physics_grid <- function(jumps_subset) {
  ...
  find_nodes_outward <- function(df, scan_direction = "right") {
    ...
    # Weighted Standard Deviation (System Spread)
    x_mean <- sum(df$x * df$y) / sum(df$y)
    x_sd   <- sqrt(sum(df$y * (df$x - x_mean)^2) / sum(df$y))
    
    # Gabor Linear Uncertainty (1/4 pi) with moderated adaptive scaling
    thresh <- (1 / (4 * pi)) / (1 + sqrt(x_sd))

    # Significance Filter: Discard noise using the relative Gabor limit
    if (h_range < (thresh * h_avg)) return(NULL)
    ...
    for (i in seq_len(nrow(df) - 1)) {
      # Net local change relative to dynamic range
      ...
      if (ready_to_fire) {
        if (accumulator >= thresh) {
          ...
          ready_to_fire <- FALSE
          # Momentum Handover: Preserve excess rise for the immediate fall check
          accumulator <- accumulator - thresh 
        }
        # Anchor to trough (Floor)
        if (accumulator < 0) accumulator <- 0
      } 
      
      # 2. FALL DETECTION (Non-blocking: checked in the same iteration)
      if (!ready_to_fire) {
        if (accumulator <= -thresh) {
          ready_to_fire <- TRUE
          # Momentum Handover: Preserve excess fall for the next rise check
          accumulator <- accumulator + thresh
        }
        # Anchor to peak (Ceiling)
        if (accumulator > 0) accumulator <- 0
      }
    }
    return(nodes)
      ...
      if (file.exists(f_path) && current_p >= 1.0) {
        ...
        if (is.null(res_r) || is.null(res_l)) {
          ...
          if(nrow(res_r) > 0 && nrow(res_l) > 0) {
            in_l <- res_l[which.max(res_l$x), ]
            in_r <- res_r[which.min(res_r$x), ]
            if(y_center <= in_l$y && y_center <= in_r$y) {
              center_dot <- data.frame(x = 0, y = y_center)
              dot_df <- rbind(center_dot, res_l[res_l$x < in_l$x, ], res_r[res_r$x > in_r$x, ])
            } else {
        ...
        p_obj <- ggplot2::ggplot(plot_df, ggplot2::aes(x = x, y = y)) +
          ...
          ggplot2::labs(title = paste0(jumps_subset[[j]]$n, " (", node_label, ") at P = ", sprintf("%.2f", current_p)), x = "q", y = "") +
  ...
  patchwork::wrap_plots(plot_list, ncol = 3) + 
    patchwork::plot_annotation(
      ...
      subtitle = "NON-BLOCKING SYMMETRIC HYSTERESIS (1/4π)",
...
```{r plot-single-histogram}
# 1. Configuration
target_p  <- 27.29
hist_path <- here::here("data-raw", "outputs", "02_aggregated_summary", "histograms", 
                        sprintf("histogram_P_%013.6f.csv.gz", target_p))

if (file.exists(hist_path)) {
  combined_dt <- data.table::fread(hist_path)
  count_val   <- combined_dt$node_count[1]
  hist_data   <- combined_dt[type == "hist"]
  node_data   <- combined_dt[type == "node"]

  # Calculate the exact bin width to ensure bars touch perfectly
  bin_width <- if(nrow(hist_data) > 1) diff(hist_data$x[1:2]) else 0.1

  ggplot(hist_data, aes(x = x, y = y)) +
    # Reference axis
    geom_vline(xintercept = 0, color = "grey80", linetype = "dashed", linewidth = 0.4) +
    
    # THE FIX: geom_col creates the "stepped" area look perfectly 
    # and is guaranteed to be "gap-free" on all ggplot2 versions.
    geom_col(fill = "black", alpha = 0.25, width = bin_width) +
    
    # The Outline: Standard 'hv' step sits right on top of the bars
    geom_step(direction = "hv", color = "black", linewidth = 0.4) +
    
    # Precision Node Markers
    geom_point(data = node_data, color = "red", size = 1.8, shape = 16) +
    
    labs(
      title = paste0("Momentum P = ", sprintf("%.2f", target_p)),
      subtitle = paste0("Detected Node Count (η) = ", count_val),
      x = "Action-Scaled Fluctuation (q)",
      y = "Occupancy / Frequency"
    ) +
