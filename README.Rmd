...
generate_plot_grid <- function(target_data, plot_title, file_name, show_placeholders = FALSE) {
  ...
  for (i in seq_len(nrow(target_data))) {
    ...
    if (file.exists(f_path)) {
      ...
      h_pts <- d_temp[type == "density"] # type 'density' matches worker output
...
# Force numerical matching to the expected resolution grid from R scripts
granularity_p <- 0.01
...
jumps <- lapply(seq_along(jump_indices), function(i) {
  ...
  list(
    ...
    # Use 0.01 granularity to see the neighborhood of the jump
    p_vals = c(m_at - 0.01, m_at, m_at + 0.01) 
  )
})
...
dt_median_approx <- data.table::rbindlist(lapply(missing_nodes, function(node_count) {
  ...
  if (nrow(candidates) > 0) {
    ...
    data.table::data.table(n = node_count, momentum = candidates$p_val[median_idx], is_approximation = TRUE)
  } else {
    data.table::data.table(n = node_count, momentum = NA_real_, is_approximation = TRUE)
  }
}))

dt_hybrid_eigenstates <- rbind(dt_eigen_exact[, is_approximation := FALSE], dt_median_approx, fill = TRUE)
...
p_momentum_hybrid <- ggplot(plot_nodes_p, aes(x = p_val, y = nodes)) +
  ...
  geom_vline(xintercept = p_markers, color = "grey80", linetype = "dashed", alpha = 0.5) +
  geom_point(data = dt_final[is_approximation == FALSE], aes(x = momentum, y = n), color = "red", size = 1.5) +
  geom_point(data = dt_final[is_approximation == TRUE], aes(x = momentum, y = n), color = "orange", size = 1.5, shape = 17) +
  geom_path(data = dt_final, aes(x = momentum, y = n), color = "darkred", linewidth = 0.5, alpha = 0.6) +
  geom_line(data = data.table::data.table(nodes = 0:max_nodes_for_plot, 
                                          p_ideal = (0:max_nodes_for_plot) * pi)[p_ideal <= max_p_observed],
            aes(x = p_ideal, y = nodes), color = "blue", linetype = "dotted", alpha = 0.5, inherit.aes = FALSE) +
  labs(
    title = latex2exp::TeX("Schrödinger Eigenstates with Median Approximations"),
    subtitle = latex2exp::TeX("Red: Exact ($P = \\pi n$) | Orange: Median Approx | Blue: Theoretical"),
    x = latex2exp::TeX("Momentum $P$ ($p/p_0$)"),
...
dt_hybrid_for_distributions <- dt_final[, .(n, momentum, is_approximation)]
...
ggplot(momentum_counts, aes(x = as.factor(n), y = Count)) +
  ...
  labs(title = "Data Availability per Quantum Number (n)", x = "Node Count", y = "Momentum entries") +
  coord_flip() +
