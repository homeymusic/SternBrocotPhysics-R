...
find_nodes_outward <- function(df, scan_direction = "right") {
  ...
  h_range <- max(df$y, na.rm = TRUE) - min(df$y, na.rm = TRUE)
  h_avg   <- mean(df$y, na.rm = TRUE)
  
  # Calculate Positional Standard Deviation (spread of the wavefunction)
  # We weight the x-values by the counts (y) to get the physical spread
  x_mean <- sum(df$x * df$y) / sum(df$y)
  x_sd   <- sqrt(sum(df$y * (df$x - x_mean)^2) / sum(df$y))
  
  # Gabor Constant (1/4 pi)
  gabor_const <- 1 / (4 * pi)
  
  # ADAPTIVE THRESHOLD: Inversely relate threshold to spread.
  # As spread (SD) increases, we become MORE sensitive to small ripples.
  # (Added a small 1+ term to prevent division by zero or excessive sensitivity)
  thresh <- gabor_const / (1 + sqrt(x_sd))

  # Significance Filter (Relative to mean)
  ...
  for (j in seq_len(num_jumps)) {
    for (p in seq_along(jumps_subset[[j]]$p_vals)) {
      ...
      if (file.exists(f_path) && current_p >= 1.0) {
        ...
        # 2. Logic to handle Noise (NA) vs Ground State (0) vs Nodes
        if (is.null(res_r) || is.null(res_l)) {
          node_label <- "NA"
          dot_df <- data.frame(x = numeric(0), y = numeric(0))
        } else {
          # Bimodal Trough Unification Logic
          if(nrow(res_r) > 0 && nrow(res_l) > 0) {
            ...
            if(y_center <= inner_l$y && y_center <= inner_r$y) {
              ...
              dot_df <- rbind(center_dot, res_l[res_l$x < inner_l$x, ], res_r[res_r$x > inner_r$x, ])
            } else {
  ...
  patchwork::wrap_plots(plot_list, ncol = 3) + 
    patchwork::plot_annotation(
      ...
      subtitle = "GABOR UNCERTAINTY (1/4π) NODE ANALYSIS",
      theme = ggplot2::theme(
