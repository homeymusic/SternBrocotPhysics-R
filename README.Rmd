...
summary_path <- here::here("data-raw", "outputs", "02_quantum_harmonic_oscillator", "bak_02_aggregated_summary.csv.gz")
...
dt_hybrid_for_distributions <- data.table::data.table(
  ...
  is_approximation = dt_final$is_approximation  # Add this line
)
...
prepare_plot_data <- function(dt_summary) {
  ...
  plot_data[, is_eigenstate := (floor(p_val / (2)) == nodes)]
  return(plot_data)
}
...
# Generate 30 evenly spaced momentum values between 0.5 and 15
target_momenta <- seq(from = 0.5, to = 15, length.out = 30)

# Find the indices in your summary table that are closest to these target values
# This uses 'match' or 'findInterval' logic to grab the nearest available data point
idx <- sapply(target_momenta, function(p_val) {
  which.min(abs(dt_summary$max_momentum - p_val))
})

# Use unique() just in case two target momenta map to the same data row
sample_momenta <- dt_summary[unique(idx), max_momentum]
