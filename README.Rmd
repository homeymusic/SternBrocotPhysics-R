...
generate_physics_grid <- function(jumps_subset) {
  # --- INTERNAL HELPER: CUMULATIVE STATE-BASED DETECTION ---
  find_nodes_outward <- function(df, scan_direction = "right") {
    # Return NULL specifically for noise-level distributions
    ...
    # 1. SIGNIFICANCE FILTER: Use Gabor uncertainty to identify pure noise
    if (h_range < (thresh * h_avg)) return(NULL)

    # 2. DETECTOR: Returns empty df (0 nodes) or populated df (>0 nodes)
