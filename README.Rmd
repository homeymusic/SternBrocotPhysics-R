...
# 1. Setup Plotting Grid using base R graphics functions
graphics::par(mfrow = c(length(jumps), 3), mar = c(4, 4, 3, 1), oma = c(2, 2, 5, 1))
...
for (j in seq_along(jumps)) {
  for (p in seq_along(jumps[[j]]$p_vals)) {
    ...
    f_name <- base::sprintf("micro_macro_erasures_P_%013.6f.csv.gz", base::round(current_p, 6))
    f_path <- here::here("data-raw", "outputs", "01_micro_macro_erasures", f_name)

    # CRITICAL FILTER: Check file existence AND P >= 1.0 rule
    if (base::file.exists(f_path) && current_p >= 1.0) {
      # Use data.table::fread explicitly
      ...
      f_range <- base::range(raw_fluc, na.rm = TRUE)
      
      # FIX: Explicitly use the min (f_range[1]) and max (f_range[2]) values
      panel_breaks <- base::seq(from = f_range[1], to = f_range[2], length.out = 402)
      
      # Generate histogram object (plot=FALSE) to get the counts
      h <- graphics::hist(raw_fluc, breaks = panel_breaks, plot = FALSE)
      
      # Create the dynamic main title string
      plot_title <- base::paste0(
        jumps[[j]]$n, " at P = ", base::round(current_p, 2)      )
      ...
      graphics::plot.new()
      graphics::text(0.5, 0.5, base::paste("P =", current_p, "\n excluded"), cex = 0.8)
    }
  }
}

# 3. Matrix Annotations 
graphics::mtext("Automated Detection of Transition Phases (P >= 1.0)", side = 3, line = 2, outer = TRUE, cex = 1.5, font = 2)
graphics::mtext("LOWER PLATEAU", side = 3, line = 0, outer = TRUE, adj = 0.15, font = 2, col = "steelblue")
graphics::mtext("JUMP POINT", side = 3, line = 0, outer = TRUE, adj = 0.5, font = 2, col = "firebrick")
graphics::mtext("UPPER PLATEAU", side = 3, line = 0, outer = TRUE, adj = 0.85, font = 2, col = "steelblue")
...
target_cols <- c(
  "zurek_entropy", "kolmogorov_complexity", "shannon_entropy"
  # ,
  # "denominator",
  # "fluctuation", "numerator"
)

render_plot <- function(data, metric_name, vlines) {
  # 1. Map columns (added min and max)
  ...
  ggplot(plot_dt, aes(x = momentum)) +
    # Reference lines (vlines)
    geom_vline(xintercept = vlines[vlines >= 1.0], color = "grey70", linetype = "dashed", alpha = 0.5) +
    
    # Subtle Min/Max Lines (Added here)
    # Using a light grey and very thin linewidth for a 'subtle' effect
    geom_line(aes(y = y_min), color = "grey80", linewidth = 0.4) +
    geom_line(aes(y = y_max), color = "grey80", linewidth = 0.4) +
    ...
    labs(title = tools::toTitleCase(gsub("_", " ", metric_name)), 
         subtitle = "Dashed lines: Zurek transitions; Thin grey lines: Min/Max range",
...
ggplot(dt_summary, aes(x = shannon_entropy_mean, y = kolmogorov_complexity_mean, color = momentum)) +
...
render_dual_plot <- function(metric_name, data_summary, transition_markers) {
  ...
  p_main <- ggplot(plot_dt, aes(x = momentum)) +
    ...
    labs(
      ...
      subtitle = bquote("Holographic Fit:" ~ y %prop% sqrt(P) ~ (k ~ .(round(k_fit, 3)))),
