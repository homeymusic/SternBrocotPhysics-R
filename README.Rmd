...
# This function creates the plot object and saves the PDF, but doesn't print it.
generate_plot_object <- function(target_data, plot_title, file_name) {
  ...
  for (i in seq_len(nrow(target_data))) {
    ...
    # --- Using your original, working sprintf format ---
...
# --- 2. Data Preparation (Median Momenta using robust indexing) ---

# A. Base Data Prep (as in your working snippet)
plot_nodes_p <- dt_summary[momentum > 0.0, .(p_val = as.numeric(momentum), nodes = as.numeric(node_count))]

# B. Full Collection (All n from 0 to 100) using robust indexing
# Select the LOWER of the two middle indices if even (.I[floor((.N+1)/2)])
dt_full <- plot_nodes_p[nodes <= 100]
data.table::setorder(dt_full, nodes, p_val)
dt_full_median <- dt_full[dt_full[, .I[floor((.N+1)/2)], by = nodes]$V1][, .(n = nodes, momentum = p_val)]


# C. Eigenstate Collection (P/pi logic) using robust indexing
dt_eigen <- plot_nodes_p[nodes <= 100 & floor(p_val/pi) == nodes]
...
# --- 3. Generate Plots and Capture Dynamic Heights ---
render_full <- generate_plot_object(dt_full_median, "Full Distribution Sequence (n=0-100, Lower Median P)", "all_nodes_median_lower.pdf")
...
# This chunk displays the full 0-100 grid with correct height in the Rmd output
...
# This chunk displays the Eigenstates grid with correct height in the Rmd output
