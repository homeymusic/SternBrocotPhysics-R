...
prepare_plot_data <- function(dt_summary) {
  ...
  plot_data[, is_eigenstate := (floor(p_val / (2*pi)) == nodes)]
  return(plot_data)
}
...
generate_plot_grid <- function(target_data, plot_title, file_name, show_placeholders = FALSE) {
  ...
  for (i in seq_len(nrow(target_data))) {
    ...
    if (file.exists(f_path)) {
      ...
      title_label  <- if(is_approx) paste0("n=", cur_n, " (Approx) | P=", round(cur_p, 3)) 
                      else paste0("n=", cur_n, " | P=", round(cur_p, 3))
...
n_cutoff <- floor(max_p_observed / (2*pi)) 
...
p_momentum_hybrid <- ggplot(plot_nodes_p, aes(x = p_val, y = nodes)) +
  ...
  geom_line(data = data.table::data.table(nodes = 0:max_nodes_for_plot, 
                              p_ideal = (0:max_nodes_for_plot) * 2*pi)[
  ...
  labs(
    ...
    caption = "Red: Exact States | Orange: Median Approx. | Blue: Ideal Trend | Grey: Full Phase Space"
  ) +
  theme_minimal() +
  theme(
