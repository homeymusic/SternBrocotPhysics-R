...
erasure_distance_density_dir  <- file.path(base_dir, "02_erasure_distance_densities")
observed_state_density_dir  <- file.path(base_dir, "02_observed_state_densities")
...
```{r combined-grid-plot-erasure-distance, echo=FALSE, fig.width=14, fig.height=25, warning=FALSE, message=FALSE}
# --- Plot 1: Combined Grayscale Small Multiples ---
render_combined_erasure_distance_grid <- function(matches_dt) {
  plot_list <- list()
  ...
  for (i in seq_len(nrow(matches_dt))) {
    ...
    if (file.exists(f_path)) {
      ...
      if (nrow(dt_found) > 0) {
        p_scat <- ggplot(dt_found, aes(x = microstate, y = minimal_action_state)) +
    ...
    if (i == 1) p_scat <- p_scat + labs(title = "Observed vs. Microstate") + 
        theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
    
    # ---------------------------------------------------------
    # 3. Coordinate Density (Right Column)
    # ---------------------------------------------------------
    d_path <- file.path(erasure_distance_density_dir, sprintf("erasure_distance_density_P_%s.csv.gz", q_str))
...
# Execute the plot
render_combined_erasure_distance_grid(dt_selected)
```

```{r combined-grid-plot-observed-state, echo=FALSE, fig.width=14, fig.height=25, warning=FALSE, message=FALSE}
# --- Plot 1: Combined Grayscale Small Multiples ---
render_combined_observed_state_grid <- function(matches_dt) {
  plot_list <- list()
  ...
  for (i in seq_len(nrow(matches_dt))) {
    ...
    if (i == 1) p_ell <- p_ell + labs(title = "Conjugate Areas") + 
        theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
    ...
    if (file.exists(f_path)) {
      ...
      if (nrow(dt_found) > 0) {
        # Strictly using 'minimal_action_state' as requested
        p_scat <- ggplot(dt_found, aes(x = microstate, y = minimal_action_state)) +
          geom_point(alpha = 0.25, size = 0.1, color = "black") + 
          theme_minimal() +
          theme(axis.title = element_blank(), 
    ...
    # 3. Coordinate Density (Right Column)
    # ---------------------------------------------------------
    d_path <- file.path(observed_state_density_dir, sprintf("observed_state_density_P_%s.csv.gz", q_str))
    ...
    if (file.exists(d_path)) {
      ...
      if (nrow(dt_active) > 0) {
        ...
        p_den <- ggplot() +
          geom_col(data = dt_active, aes(x = pct_q, y = pct_density), width = pct_width * 0.8, fill = "gray50") +
          coord_cartesian(xlim = c(-100, 100), ylim = c(0, 105)) +
          theme_minimal() +
          theme(axis.title = element_blank(), 
                axis.text = element_blank(),
    ...
    if (i == 1) p_den <- p_den + labs(title = "Observed State Density") + 
        theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
  ...
  final_grid <- wrap_plots(plot_list, ncol = 3) +
    plot_annotation(
      title = "Structural Evolution of Observed States",
...
# Execute the plot
render_combined_observed_state_grid(dt_selected)
```
...
render_micro_macro_scatters <- function(matches_dt) {
  for (i in seq_len(nrow(matches_dt))) {
    ...
    if (file.exists(f_path)) {
      ...
      p <- ggplot(dt_found, aes(x = microstate, y = minimal_action_state)) +
        ...
        labs(title = sprintf("Phase Space Erasure: P = %.6f", q_val),
             ...
             x = "Microstate", y = "minimal_action_state") +
        theme_minimal() +
...
# --- Plot 5: Micro Erasure ---
render_micro_erasure_scatters <- function(matches_dt) {
  for (i in seq_len(nrow(matches_dt))) {
    row <- matches_dt[i]
    q_val <- row$normalized_momentum
    q_str <- sprintf("%013.6f", q_val)
...
render_micro_erasure_scatters(dt_selected)

# --- Plot 6: Macro Erasure ---
render_macro_erasure_scatters <- function(matches_dt) {
  for (i in seq_len(nrow(matches_dt))) {
    row <- matches_dt[i]
    q_val <- row$normalized_momentum
    q_str <- sprintf("%013.6f", q_val)
    ...
    if (file.exists(f_path)) {
      ...
      p <- ggplot(dt_found, aes(x = minimal_action_state, y = erasure_distance)) +
        geom_point(alpha = 0.4, size = 0.5, color = "black") +
        labs(title = sprintf("Phase Space Erasure: P = %.6f", q_val),
             subtitle = sprintf("Nodes: %d | Range: [%.3f, %.3f]", 
                               row$node_count, min(dt_found$microstate), max(dt_found$microstate)),
             x = "minimal_action_state", y = "Erasure Distance") +
        theme_minimal() +
        theme(plot.title = element_text(family = "mono", face = "bold"))
...
render_macro_erasure_scatters(dt_selected)

# --- Plot 7: Erasure Macro ---
render_erasure_macro_scatters <- function(matches_dt) {
  for (i in seq_len(nrow(matches_dt))) {
    row <- matches_dt[i]
    q_val <- row$normalized_momentum
    q_str <- sprintf("%013.6f", q_val)
    ...
    if (file.exists(f_path)) {
      ...
      p <- ggplot(dt_found, aes(x = erasure_distance, y = minimal_action_state)) +
        geom_point(alpha = 0.4, size = 0.5, color = "black") +
        labs(title = sprintf("Phase Space Erasure: P = %.6f", q_val),
             subtitle = sprintf("Nodes: %d | Range: [%.3f, %.3f]", 
                               row$node_count, min(dt_found$microstate), max(dt_found$microstate)),
             x = "Erasure Distance", y = "minimal_action_state") +
        theme_minimal() +
        theme(plot.title = element_text(family = "mono", face = "bold"))
...
render_erasure_macro_scatters(dt_selected)

# --- Plot 8: Conjugate Ellipses ---
