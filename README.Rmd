...
p_spatial <- ggplot(plot_dt_q, aes(x = q_val)) +
  ...
  labs(
    title = latex2exp::TeX("Coordinate Complexity: $K(Q_0)$ vs. $Q$"),
...
```
```{r filter-valid-eigenstates-holographic, echo=FALSE, fig.width=8, fig.height=6}
...
max_rows_to_plot <- 20  # How many quantum numbers (rows) to show
cols_per_row     <- 3
...
```{r eigen-n4-distributions, echo=FALSE, warning=FALSE, message=FALSE}
# --- 1. FILTER FOR SPECIFIC QUANTUM NUMBER ---
target_n <- 3

# Extract all valid eigenstates from the original data
eigen_momenta_all <- plot_nodes_p[is_eigenstate == TRUE, .(
  n = as.integer(nodes),
  momentum = as.numeric(p_val)
)]

# Isolate all momenta for n == 4
subset_n4 <- eigen_momenta_all[n == target_n]

# Sort by momentum so the sequence is logical (low P to high P)
data.table::setorder(subset_n4, momentum)

# DYNAMIC HEIGHT: 3 units per row, assuming 3 columns
cols_per_row  <- 3
dynamic_h_n4  <- ceiling(nrow(subset_n4) / cols_per_row) * 3
```

```{r plot-n4-grid, echo=FALSE, fig.width=10, fig.height=dynamic_h_n4, warning=FALSE, message=FALSE}
# --- 2. GENERATE GRID ---
plot_list <- vector("list", nrow(subset_n4))

for (i in seq_len(nrow(subset_n4))) {
  current_p <- subset_n4$momentum[i]
  ...
  if (file.exists(f_path)) {
    ...
    plot_list[[i]] <- ggplot2::ggplot(hist_data, ggplot2::aes(x = x, y = y)) +
      ...
      ggplot2::geom_col(fill = "black", alpha = 0.25, width = b_width) +
      ggplot2::geom_step(direction = "hv", color = "black", linewidth = 0.3) +
      ggplot2::geom_point(data = node_data, color = "red", size = 1.2, shape = 16) +
      ggplot2::labs(
        title = paste0("n = ", target_n, " | P = ", sprintf("%.6f", current_p)), 
        x = "q", y = ""
      ) +
      ggplot2::theme_minimal(base_family = "mono") +
      ggplot2::theme(
...
# Assemble with Patchwork
n4_plot <- patchwork::wrap_plots(plot_list, ncol = cols_per_row) + 
  patchwork::plot_annotation(
    title = paste("Full Resonant Spectrum for n =", target_n),
    subtitle = paste("Showing all", nrow(subset_n4), "valid momenta configurations found"),
    theme = ggplot2::theme(
      plot.title = ggplot2::element_text(hjust = 0.5, face = "bold", size = 14, family = "mono")
    )
  )
...
# Save
ggplot2::ggsave(
  filename = file.path(manuscript_dir, paste0("eigen_spectrum_n", target_n, ".pdf")),
  plot   = n4_plot, 
  width  = 10, 
  height = dynamic_h_n4,
  device = grDevices::cairo_pdf,
