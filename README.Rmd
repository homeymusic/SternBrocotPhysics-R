...
# APPLY CONSTRAINTS: P <= 4.0 and n <= 10 (and the physical floor P >= 0.5)
dt_summary <- dt_summary[normalized_momentum >= 0.5 & 
                         normalized_momentum <= 4.0 & 
                         node_count <= 10]
  ...
  for (i in seq_len(nrow(matches_dt))) {
    ...
    if (file.exists(d_path)) {
      ...
      p <- ggplot(h_pts, aes(x = coordinate_q, y = density_count)) +
        ...
        labs(
          title = sprintf("Traditional Physics: n = %d", row$node_count),
          subtitle = sprintf("P = %.6f | Deviation: %.6f", p_val, row$deviation),
