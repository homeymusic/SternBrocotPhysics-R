...
find_nodes_outward <- function(df, scan_direction = "right") {
  ...
  # The ONLY threshold used for all logic
  thresh <- 1 / (4 * pi)
  ...
  for (i in seq_len(nrow(df) - 1)) {
    # Net local change relative to global range
    local_change <- (df$y[i+1] - df$y[i]) / h_range
    accumulator  <- accumulator + local_change
    
    if (ready_to_fire) {
      # 1. Searching for Rise: Cumulative net change must exceed 1/(4*pi)
      if (accumulator >= thresh) {
        nodes <- rbind(nodes, df[i+1, c("x", "y")])
        ready_to_fire <- FALSE
        accumulator   <- 0 # Reset to track the required fall
      }
      # Clamp floor: If we are 'falling' while looking for a rise, reset to 0
      if (accumulator < 0) accumulator <- 0
      
    } else {
      # 2. Searching for Fall (Symmetrical Reset): 
      # Must fall by the same 1/(4*pi) relative to the node just found
      if (accumulator <= -thresh) {
        ready_to_fire <- TRUE
        accumulator   <- 0 # Reset to track the next rise
      }
      # Clamp ceiling: If we are still rising after firing, reset to 0
      if (accumulator > 0) accumulator <- 0
    }
  }
  return(nodes)
