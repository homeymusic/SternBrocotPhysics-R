...
# Matches the %013.6f and 'density' naming convention from generation scripts
# Updated to ensure string-perfect lookup for 0.001 resolution
get_histogram_path <- function(momentum_val) {
  file.path(agg_dir, sprintf("density_P_%013.6f.csv.gz", momentum_val))
}
...
generate_plot_grid <- function(target_data, plot_title, file_name, show_placeholders = FALSE) {
  ...
  for (i in seq_len(nrow(target_data))) {
    ...
    if (file.exists(f_path)) {
      ...
      h_pts <- d_temp[type == "density"] # type 'density' matches 02 aggregator
  ...
  for (j in seq_len(num_jumps)) {
    for (p in seq_along(jumps_subset[[j]]$p_vals)) {
      ...
      if (file.exists(f_path)) {
        ...
        hist_data <- combined_dt[type == "density"]
        ...
        p_obj <- ggplot2::ggplot(hist_data, ggplot2::aes(x = x, y = y)) +
          ...
          ggplot2::labs(
            title = paste0(jumps_subset[[j]]$n, " Nodes: ", node_label, " at P = ", sprintf("%.4f", current_p)), 
            x = "q", y = ""
          ) +
          ggplot2::theme_minimal(base_family = "mono") +
          ggplot2::theme(
        ...
        p_obj <- ggplot2::ggplot() + ggplot2::theme_void() +   
          ggplot2::annotate("text", x = 0.5, y = 0.5, 
                            label = paste("P =", sprintf("%.4f", current_p), "\nmissing"),
                            family = "mono", size = 3)
      }
...
# Selecting specific columns for performance with 300k+ rows
dt_summary <- data.table::fread(summary_path, 
                                select = c("normalized_momentum", "program_length_median", 
                                           "node_count", "program_length_mean", "program_length_sd"))
data.table::setorder(dt_summary, normalized_momentum)

# Force numerical matching to the new high-resolution grid (0.001)
granularity_p <- 0.001
dt_summary <- dt_summary[abs(normalized_momentum %% granularity_p) < 1e-9]
...
jumps <- lapply(seq_along(jump_indices), function(i) {
  ...
  list(
    ...
    # Use 0.001 granularity to look at immediate local neighborhood of the jump
    p_vals = c(m_at - 0.001, m_at, m_at + 0.001) 
  )
})
...
# Calculate the physical limit based on max momentum observed
...
# For missing nodes, get the median
...
ggplot(momentum_counts, aes(x = as.factor(n), y = Count)) +
...
```{r experimental_ellipses, fig.width=10, fig.height=dynamic_h_ellipse, echo=FALSE}
ellipse_data <- lapply(jumps, function(j) {
  data.frame(
    transition_name = j$n,
    P_value = j$p_vals,
    Q_value = j$p_vals,
    p_label = factor(sprintf("P = Q = %.4f", j$p_vals), 
                     levels = sprintf("P = Q = %.4f", j$p_vals))
  )
}) %>% bind_rows()
...
# Calculate dynamic height: 3.5 inches per row is the sweet spot for 3-column facets
num_transitions <- length(unique(ellipse_data$transition_name))
dynamic_h_ellipse <- max(4, num_transitions * 3.5) 
