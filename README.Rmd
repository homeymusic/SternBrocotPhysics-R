...
for (metric in target_cols) {
  if (paste0(metric, "_mean") %in% names(dt)) {
    print(render_plot(dt, metric, p_markers))
  }
}
...
plot(
  ...
  xlab = expression(paste("Microstate (", x["?"], ")")),
  ylab = expression(paste("Kolmogorov Complexity (", K, ") [bits]")),
  pch = ".",
...
```{r plot-zurek-entropy, fig.width=10, fig.height=6, echo=F}
plot(
  x = df$microstate,
  y = df$zurek_entropy,
  main = expression(paste("Zurek Entropy (", H, ") vs Microstate (", x["?"], ")")),
  xlab = expression(paste("Microstate (", x["?"], ")")),
  ylab = expression(paste("Zurek Entropy (", Z, ") [bits]")),
  pch = ".",
  col = rgb(0.1, 0.6, 0.1, 0.1), # A green color with transparency
  cex = 0.5
)
