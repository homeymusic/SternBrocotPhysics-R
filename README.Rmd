...
# 2. Visualization (Starting from a clean slate)
p_main_m <- ggplot(plot_dt, aes(x = momentum)) +
  ...
  # Using standard theme without fixed aspect ratios
  theme_minimal() + 
  theme(legend.position = "bottom")

# 3. Save (Standard 8x6 is a common "natural" default)
ggplot2::ggsave(
  filename = file.path(manuscript_dir, "landauer_momentum_reset.pdf"),
  plot = p_main_m, 
  width = 8, 
  height = 6, 
...
# 1. Prepare data
plot_nodes <- dt_summary[momentum > 0.0, .(
  momentum = as.numeric(momentum),
...
```{r plot-complexity-node-count, echo=FALSE}
# 1. Prepare Merged Data
plot_combined <- dt_summary[momentum > 0.0, .(
...
# 2. Visualization with Integer-Only X-Axis
ggplot(plot_combined, aes(y = nodes)) +
  geom_point(aes(x = a_mean, color = "Mean"), size = 0.5, alpha = 0.5) +
  geom_point(aes(x = a_med, color = "Median"), size = 1) +
  # Force X-axis to only show whole numbers
  scale_x_continuous(breaks = function(x) seq(floor(min(x)), ceiling(max(x)), by = 1)) +
  scale_color_manual(values = c("Mean" = "gray30", "Median" = "black")) +
  labs(
    title = "Node Count Scaling with Complexity",
    x = "Complexity (Integer Measures)",
    y = "Node Count",
    color = "Measure"
  ) +
  theme_minimal()
