...
generate_physics_grid <- function(jumps_subset) {
  ...
  find_nodes_outward <- function(df, scan_direction = "right") {
    ...
    # --- SANITY CHECK: SIGNAL-TO-NOISE RATIO ---
    # If the total variation is less than 5% of the average height, 
    # it's likely a uniform distribution with noise. Skip node detection.
    if (h_range < (0.05 * h_avg)) return(empty_result)

    thresh <- 1 / (4 * pi); reset_val <- 0 
    ...
    for (p in seq_along(jumps_subset[[j]]$p_vals)) {
      ...
      if (file.exists(f_path) && current_p >= 1.0) {
        ...
        p_obj <- ggplot2::ggplot(plot_df, ggplot2::aes(x = x, y = y)) +
          ...
          ggplot2::labs(title = paste0(jumps_subset[[j]]$n, " at P = ", sprintf("%.2f", current_p)), x = "q", y = "") +
