...
# 1. Define the Metric-to-Title Map
metric_titles <- c(
...
# Function to generate the two-panel plot
render_dual_plot <- function(metric_name, data_summary, transition_markers) {
  
  # Retrieve clean title from map; fallback to metric_name if missing
  clean_title <- metric_titles[metric_name]
  if (is.na(clean_title)) clean_title <- metric_name
  ...
  holographic_label = paste("Holographic Theory", expression(K = sqrt(P)))
  ...
  p_main <- ggplot(plot_dt, aes(x = momentum)) +
    ...
    geom_line(aes(y = theory, color = holographic_label), 
              linewidth = 0.8, alpha = 0.8, linetype = "dashed") +
    scale_color_manual(values = c("Mean" = "gray40", "Median" = "black", holographic_label = "gray60")) +
    labs(
      title = paste(clean_title, "vs. Normalized Momentum P"),
      subtitle = bquote("Holographic Fit:" ~ sqrt(P) ~ (k ~ .(round(k_fit, 3)))),
      x = latex2exp::TeX(sprintf("%s $P = p / \\sqrt{(m h \\omega) / 2\\pi}$", "Normalized Momentum")),
      y = latex2exp::TeX(sprintf("%s $[bits]$", clean_title)),
      color = "Series"
    ) +
    theme_minimal() +
    theme(
  ...
  combined <- cowplot::plot_grid(p_main, p_residuals, ncol = 1, align = "v", rel_heights = c(3, 1))
  return(combined)
}
