...
for (val_n in target_nodes) {
  if (val_n == 0) {
    ...
    if (nrow(ground_state) > 0) {
      dt_eigenstate_matches <- rbind(dt_eigenstate_matches, ground_state[1])
    }
  } else {
    # SELECT BY COMPLEXITY FOR ALL OTHER NODES
...
p_ov_action <- ggplot() +
  ...
  scale_y_continuous(breaks = seq(0, max_n, by = 1), expand = expansion(mult = c(0.05, 0.1))) +
  theme_minimal()
...
render_profiles <- function(matches_dt, dot_color = "black") {
  for (i in seq_len(nrow(matches_dt))) {
    ...
    # Path handling (standard vs fixed-width)
    ...
    if (file.exists(d_path)) {
      ...
      # 1. Surgical NZ Trim (The Animation Logic)
      ...
      # 3. Geometry: Step Outline with Padding
      ...
      # 4. Advanced Node Centering (The "Plateau" logic)
