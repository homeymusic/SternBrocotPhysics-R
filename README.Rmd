...
get_histogram_path <- function(momentum_val) {
  here::here("data-raw", "outputs", "02_aggregated_summary", "histograms", 
...
prepare_plot_data <- function(dt_summary) {
  plot_data <- dt_summary[momentum > 0.0, .(
...
momentum_counts <- dt_summary[momentum > 0.0, .(Count = .N), by = .(n = as.integer(node_count))]
