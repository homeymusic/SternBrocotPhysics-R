...
## 2. Representative Diagnostic Scan (One Median Plot per Node Count)
```{r diagnostic-scan, echo=FALSE, results='asis', fig.width=8, fig.height=4}
# 1. Selection Logic: 
# Filter out NA node_counts, then prioritize Eigenstates, then lowest momentum
dt_representatives <- dt_summary[!is.na(node_count)][
  order(node_count, -is_eigenstate, normalized_momentum)
][, .SD[1], by = node_count]
...
for (i in seq_len(nrow(dt_representatives))) {
  ...
  # Ensure is_eig is not NA before the if() check (extra safety)
  ...
  if (file.exists(d_path)) {
    ...
    # Using the safe boolean check
    type_label <- if(is_eig_safe) "Eigenstate" else "Lowest Momentum (Non-Eigenstate)"
