    ...
    for (p in seq_along(jumps_subset[[j]]$p_vals)) {
      ...
      if (file.exists(f_path)) {
        ...
        p_obj <- ggplot2::ggplot(hist_data, ggplot2::aes(x = x, y = y)) +
          ...
          ggplot2::labs(
            title = paste0(jumps_subset[[j]]$n, " (", node_label, ") at P = ", sprintf("%.2f", current_p)), 
            x = "q", y = ""
          ) +
  ...
  patchwork::wrap_plots(plot_list, ncol = 3) + 
    patchwork::plot_annotation(
      ...
      theme = ggplot2::theme(
        ...
        plot.subtitle = ggplot2::element_text(hjust = 0.5, size = 10, family = "mono", color = "red")
      )
    )
}
```
...
```{r plot-single-histogram, fig.width=8, fig.asp=0.6, out.width="100%"}
# 1. Configuration
target_p  <- 25.24
...
if (file.exists(hist_path)) {
  ...
  ggplot(hist_data, aes(x = x, y = y)) +
    ...
    labs(
      title = paste("Momentum P:", sprintf("%.2f", target_p)),
      subtitle = paste("Detected Node Count:", count_val),
      x = "q",
      y = "Count at q"
    ) +
