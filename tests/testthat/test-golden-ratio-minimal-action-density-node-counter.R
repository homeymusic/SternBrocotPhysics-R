...
test_that("Golden Ratio minimal action density node counts match theoretical expectations", {
  ...
  fixtures_dir    <- test_path("fixtures_golden_ratio_minimal_action")
  debug_plots_dir <- test_path("_debug_plots_golden_ratio_minimal_action")
  debug_data_dir  <- test_path("_debug_data_golden_ratio_minimal_action")
  temp_raw_dir    <- file.path(tempdir(), "raw_sim_golden_ratio_minimal_action")
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    fixture_path <- file.path(fixtures_dir, sprintf("harmonic_oscillator_minimal_action_state_density_golden_ratio_P_%s.csv.gz", m_str))
    ...
    if (file.exists(raw_file)) {
      process_action_densities(f = raw_file, out_path = fixtures_dir, target_cols = c("minimal_action_state"))
      unlink(raw_file)
    }
    ...
    p <- ggplot() +
      ...
      labs(title = paste("Diagnostic (Golden Ratio | Minimal Action) | P =", m_val),
    ...
    if (is_mismatch) {
      pdf_path <- file.path(debug_plots_dir, sprintf("node_debug_golden_ratio_minimal_action_P_%s.pdf", m_str))
      ggsave(pdf_path, plot = p, device = "pdf", width = 8, height = 6)
      data.table::fwrite(density_data, file.path(debug_data_dir, sprintf("FAILURE_DATA_golden_ratio_minimal_action_P_%s.csv", m_str)))
    }
    ...
    vdiffr::expect_doppelganger(paste0("baseline_nodes_golden_ratio_minimal_action_P_", m_str), p)
  }
  unlink(temp_raw_dir, recursive = TRUE)
})
