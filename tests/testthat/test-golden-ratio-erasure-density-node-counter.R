...
test_that("Golden Ratio erasure density node counts match theoretical expectations", {
  ...
  debug_plots_dir <- test_path("_debug_plots_golden_ratio_erasure")
  debug_data_dir  <- test_path("_debug_data_golden_ratio_erasure")
  temp_raw_dir    <- file.path(tempdir(), "raw_sim_golden_ratio_erasure")
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    p <- ggplot() +
      ...
      labs(title = paste("Diagnostic (Golden Ratio | Erasure) | P =", m_val),
    ...
    if (is_mismatch) {
      pdf_path <- file.path(debug_plots_dir, sprintf("node_debug_golden_ratio_erasure_P_%s.pdf", m_str))
      ggsave(pdf_path, plot = p, device = "pdf", width = 8, height = 6)
      data.table::fwrite(density_data, file.path(debug_data_dir, sprintf("FAILURE_DATA_golden_ratio_erasure_P_%s.csv", m_str)))
    }
    ...
    vdiffr::expect_doppelganger(paste0("baseline_nodes_golden_ratio_erasure_P_", m_str), p)
  }
  unlink(temp_raw_dir, recursive = TRUE)
})
