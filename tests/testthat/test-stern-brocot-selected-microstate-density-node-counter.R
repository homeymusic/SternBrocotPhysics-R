...
test_that("Stern-Brocot selected microstate density node counts match theoretical expectations", {
  ...
  fixtures_dir    <- test_path("fixtures_stern_brocot_selected_microstate")
  debug_plots_dir <- test_path("_debug_plots_stern_brocot_selected_microstate")
  debug_data_dir  <- test_path("_debug_data_stern_brocot_selected_microstate")
  temp_raw_dir    <- file.path(tempdir(), "raw_sim_stern_brocot_selected_microstate")
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    # Look specifically for the stern_brocot selected_microstate processed output
    fixture_path <- file.path(fixtures_dir, sprintf("harmonic_oscillator_selected_microstate_density_stern_brocot_P_%s.csv.gz", m_str))

    # Pass the 'stern_brocot' algorithm parameter
    ...
    # Look specifically for the stern_brocot raw C++ output
    raw_file <- file.path(temp_raw_dir, sprintf("harmonic_oscillator_erasures_stern_brocot_P_%s.csv.gz", m_str))
    ...
    p <- ggplot() +
      ...
      labs(title = paste("Diagnostic (Stern-Brocot | Selected Microstate) | P =", m_val),
    ...
    if (is_mismatch) {
      pdf_path <- file.path(debug_plots_dir, sprintf("node_debug_stern_brocot_selected_microstate_P_%s.pdf", m_str))
      ggsave(pdf_path, plot = p, device = "pdf", width = 8, height = 6)

      dump_file <- file.path(debug_data_dir, sprintf("FAILURE_DATA_stern_brocot_selected_microstate_P_%s.csv", m_str))
    ...
    vdiffr::expect_doppelganger(paste0("baseline_nodes_stern_brocot_selected_microstate_P_", m_str), p)
  }
