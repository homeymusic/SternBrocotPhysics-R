...
test_that("Action-Angle erasure density node counts match theoretical expectations", {
  ...
  fixtures_dir    <- test_path("fixtures_action_angle_erasure")
  debug_plots_dir <- test_path("_debug_plots_action_angle_erasure")
  debug_data_dir  <- test_path("_debug_data_action_angle_erasure")
  temp_raw_dir    <- file.path(tempdir(), "raw_sim_action_angle_erasure")
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    p <- ggplot() +
      ...
      labs(title = paste("Diagnostic (Action-Angle | Erasure) | P =", m_val),
    ...
    if (is_mismatch) {
      pdf_path <- file.path(debug_plots_dir, sprintf("node_debug_action_angle_erasure_P_%s.pdf", m_str))
      ggsave(pdf_path, plot = p, device = "pdf", width = 8, height = 6)

      dump_file <- file.path(debug_data_dir, sprintf("FAILURE_DATA_action_angle_erasure_P_%s.csv", m_str))
    ...
    vdiffr::expect_doppelganger(paste0("baseline_nodes_action_angle_erasure_P_", m_str), p)
  }
