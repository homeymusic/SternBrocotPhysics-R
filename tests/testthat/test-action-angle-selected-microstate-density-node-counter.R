...
test_that("Action-Angle selected microstate density node counts match theoretical expectations", {
  ...
  fixtures_dir    <- test_path("fixtures_action_angle_selected_microstate")
  debug_plots_dir <- test_path("_debug_plots_action_angle_selected_microstate")
  debug_data_dir  <- test_path("_debug_data_action_angle_selected_microstate")
  temp_raw_dir    <- file.path(tempdir(), "raw_sim_action_angle_selected_microstate")
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    # Look specifically for the action_angle selected_microstate processed output
    fixture_path <- file.path(fixtures_dir, sprintf("harmonic_oscillator_selected_microstate_density_action_angle_P_%s.csv.gz", m_str))
    ...
    p <- ggplot() +
      ...
      labs(title = paste("Diagnostic (Action-Angle | Selected Microstate) | P =", m_val),
    ...
    if (is_mismatch) {
      pdf_path <- file.path(debug_plots_dir, sprintf("node_debug_action_angle_selected_microstate_P_%s.pdf", m_str))
      ggsave(pdf_path, plot = p, device = "pdf", width = 8, height = 6)

      dump_file <- file.path(debug_data_dir, sprintf("FAILURE_DATA_action_angle_selected_microstate_P_%s.csv", m_str))
    ...
    vdiffr::expect_doppelganger(paste0("baseline_nodes_action_angle_selected_microstate_P_", m_str), p)
  }
