...
test_that("Golden Ratio selected microstate density node counts match theoretical expectations", {
  ...
  fixtures_dir    <- test_path("fixtures_golden_ratio_selected_microstate")
  debug_plots_dir <- test_path("_debug_plots_golden_ratio_selected_microstate")
  debug_data_dir  <- test_path("_debug_data_golden_ratio_selected_microstate")
  temp_raw_dir    <- file.path(tempdir(), "raw_sim_golden_ratio_selected_microstate")
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    fixture_path <- file.path(fixtures_dir, sprintf("harmonic_oscillator_selected_microstate_density_golden_ratio_P_%s.csv.gz", m_str))
    ...
    if (file.exists(raw_file)) {
      process_action_densities(f = raw_file, out_path = fixtures_dir, target_cols = c("selected_microstate"))
      unlink(raw_file)
    }
    ...
    skyline_x <- c(density_data$x[1] - bw/2, rep(density_data$x, each = 2) + rep(c(-bw/2, bw/2), nrow(density_data)), density_data$x[nrow(density_data)] + bw/2)
    ...
    p <- ggplot() +
      ...
      labs(title = paste("Diagnostic (Golden Ratio | Selected Microstate) | P =", m_val),
           subtitle = sprintf("Actual: %s | Expected: %s | Bins: %d", results$node_count, expected_n, nrow(density_data))) +
      theme_minimal()

    if (!is.null(results$nodes) && nrow(results$nodes) > 0) {
      p <- p + geom_point(data = results$nodes, aes(x, y), color = "orange", size = 2)
    }

    is_mismatch <- if (is.na(expected_n)) !is.na(results$node_count) else (is.na(results$node_count) || results$node_count != expected_n)

    if (is_mismatch) {
      pdf_path <- file.path(debug_plots_dir, sprintf("node_debug_golden_ratio_selected_microstate_P_%s.pdf", m_str))
      ggsave(pdf_path, plot = p, device = "pdf", width = 8, height = 6)
      data.table::fwrite(density_data, file.path(debug_data_dir, sprintf("FAILURE_DATA_golden_ratio_selected_microstate_P_%s.csv", m_str)))
    }

    test_info <- sprintf("FAILURE P=%s (Expected: %s, Actual: %s)", m_val, expected_n, results$node_count)
    if (is.na(expected_n)) expect_true(is.na(results$node_count), info = test_info)
    else expect_equal(results$node_count, expected_n, info = test_info)

    vdiffr::expect_doppelganger(paste0("baseline_nodes_golden_ratio_selected_microstate_P_", m_str), p)
  }
  unlink(temp_raw_dir, recursive = TRUE)
})
