...
test_that("Node counts match theoretical expectations for specific momenta", {
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    # tag from the raw filename. We must look for it here!
    fixture_path <- file.path(fixtures_dir, sprintf("harmonic_oscillator_erasure_distance_density_stern_brocot_P_%s.csv.gz", m_str))
