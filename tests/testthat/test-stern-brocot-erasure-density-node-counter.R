...
test_that("Node counts match theoretical expectations for specific momenta", {
  ...
  fixtures_dir    <- test_path("fixtures_stern_brocot")
  debug_plots_dir <- test_path("_debug_plots_stern_brocot")
  debug_data_dir  <- test_path("_debug_data_stern_brocot")
  ...
  for (i in seq_len(nrow(truth_table))) {
    ...
    # CRITICAL FIX: The process_action_densities output will retain the "_stern_brocot_"
    # tag from the raw filename AND use the new target column name.
    fixture_path <- file.path(fixtures_dir, sprintf("harmonic_oscillator_erasure_displacement_density_stern_brocot_P_%s.csv.gz", m_str))

    # Pass the explicit algorithm parameter we added to C++
