...
#' @export
compute_observed_state_density <- function(dt, p, bin_width = 1) {
  # --- 1. PRE-PROCESSING ---
...
#' @return A data.table
#' @export
process_observed_state_density <- function(f, out_path) {
  tryCatch({
    library(data.table)
    setDTthreads(1)
    ...
    density_df <- compute_observed_state_density(dt, P_effective, bin_width = 1.0)
