...
#' @import data.table
#' @export
plot_strict_erasure_grid <- function(dt_meta, dir_raw, dir_densities, dir_nodes,
  ...
  for (i in seq_len(num_rows)) {
    ...
    if (file.exists(file_raw)) {
      ...
      p_scat <- ggplot(dt_raw, aes(x=x_scaled, y=y_scaled)) +
        ...
        scale_y_continuous(breaks = custom_breaks, labels = label_format) +
        theme_bw(base_family = base_font) +
        theme(panel.grid.minor=element_blank(), axis.text = element_text(size=8), axis.title.y = element_text(size=9), aspect.ratio=1) +
